...
try:
        ...
        for x1,y1,w,h in faces:
            ...
            for i in data:
                ...
                if(count==200):
                    ...
                    dfb = next(iter(df[df['Name']==live_Name].index), 'no match')
                    if(dataframe.loc[dfb,'Status']=='present'):
                        data1={"Name":[],"Status":[],"Time":[]}
                        data1["Name"].append(live_Name)
                        data1["Status"].append("present")
                        data1["Time"].append(datetime.datetime.now())
                        data2=pd.DataFrame(data1)
                        DD=dataframe.append(data2,ignore_index=True,sort=False)
                        DD=DD.drop(columns=['Unnamed: 0'])
                        print(DD)
                        DD.to_csv('Book1.csv')
                        data.clear()
                        print(live_Name,'your attendance has been recorded')
                        count=0
                    else:
                        dataframe.loc[dfb,'Status']='present'
                        print(12)
                        dataframe.loc[dfb,'Time']=datetime.datetime.now()
                        dd=dataframe.drop(columns=['Unnamed: 0'])
                        dd.to_csv('Book1.csv')
                        data.clear()
                        print(live_Name,'your attendance has been recorded')
                        count=0
    ...
    #df=pd.read_csv('Book1.csv')
    #dataframe=pd.DataFrame(df)
    #dfb = next(iter(df[df['Name']==(most_frequent(data))].index), 'no match')
    
    
    #dataframe.loc[dfb,'Time']=datetime.datetime.now()
    #if(dataframe.loc[dfb,'Status']=='present'):
   #     data1={"Name":[],"Status":[],"Time":[]}
    #    data1["Name"].append((most_frequent(data)))
     #   data1["Status"].append("present")
      #  data1["Time"].append(datetime.datetime.now())
       # data2=pd.DataFrame(data1)
        #DD=dataframe.append(data2,ignore_index=True)
        #dd=dataframe.drop(columns=['Unnamed: 0'])
        #dd.to_csv('Book1.csv')
    #else:
     #   dataframe.loc[dfb,'Status']='present'
